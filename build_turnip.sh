...
# NOTA: fixado no branch gen8 de propÃ³sito. O suporte a8xx/gen8 (fd6_gmem_cache.h
# e afins) sÃ³ existe a partir de dez/2025 - qualquer tag 25.x (incluindo 25.1.4)
# Ã© anterior a isso e quebra o driver. Ver checagem de versÃ£o em check_driver_version().
mesabranch="gen8"
srcfolder="mesa"
BUILD_VERSION="${BUILD_VERSION:-1.0}"

# Patches que aplicam limpo em cima do gen8 atual (testados manualmente, um a um E em
# sequÃªncia - houve conflito real entre dois deles, ver nota abaixo).
# Os demais patches do repo (tu_gen8.patch, tu_gen8_clean.patch, tu8_kgsl_26.patch,
# disable_vkQueueSubmit2.patch) jÃ¡ estÃ£o incorporados no branch gen8 ou duplicados
# por outro fix neste script, e por isso NÃƒO entram aqui.
#
...
prepare_workdir(){
    ...
    if [ ! -d "$srcfolder" ]; then
        echo "Baixando cÃ³digo fonte do Mesa (branch: $mesabranch)..."
        git clone "$mesasrc" --depth=1 --branch "$mesabranch" "$srcfolder"
    else
